...
public:
  static constexpr int nGridsMerge = Gs_t::nGridsMerge;
  static size_t cub_tmp_bytes;
  static size_t global_memory_limit;
  ull max_allowed_flop;
  GpuComputerAsync(const GpuComputerAsync&)=delete;
  GpuComputerAsync(GpuComputerAsync&&)=delete;
  void operator=(const GpuComputerAsync&)=delete;
  void operator=(GpuComputerAsync&&)=delete;
  bool try_initialize(size_t max_nnzA, size_t max_nnzB, size_t max_flop);
  cudaError_t addGpuTask(int rBid, int cBid, bool gBuffer);
  cudaError_t doKernel();
  cudaError_t doPtrD2H();
  cudaError_t doDataD2H();
  // void realloc_size(size_t flop);
  ~GpuComputerAsync();
  GpuComputerAsync()=delete;
  GpuComputerAsync(int nrA, int ncB, size_t max_nnzA, size_t max_nnzB, int device,
    Gs_t*gs, std::vector<std::unique_ptr<write_t>> & write_data, gpu_profile_t* gpu_profiler);
  GPUControllingBlock<EIdType> gcb;
  int counter;
  write_t *wt[2];
  gpu_profile_t *prof;
  void set_call_back_finish_counter(std::atomic_llong*counter);
...
private:
  ...
  IdxType* d_tmpIdx2;
  ValType* d_tmpVal2;
  bool buffer_visited_h2d[2];
  bool buffer_visited_decomp[2];
  bool check_device_memory_available(int p_nrA, int p_ncB, size_t p_max_nnzA, size_t p_max_nnzB, unsigned long long p_max_flop, const Gs_t* gs_ptr);
} ;
