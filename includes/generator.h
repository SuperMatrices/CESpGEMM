...
template<typename EIdType, typename ValType, typename Compressor>
inline void compress_ptr_parameterized(IdxType nr, EIdType* cPtr, Compressor &cmp, IdxType* aux, int min_zero_num, int max_zero_seg){
  int compLen = nr + 1;
  aux[0] = 0;
  for(int i=1;i<compLen;i++){
    aux[i] = cPtr[i] - cPtr[i-1];
  }
  cmp.compress_with_zero_per_seg(compLen, aux, min_zero_num, max_zero_seg);
}
