...
struct csr
{
  ...
  shared_ptr<csr<EIdType,ValType>> make_borrowed(){
    shared_ptr<csr<EIdType,ValType>> ret = std::make_shared<csr<EIdType,ValType>>();
    *ret = std::move(*this);
    return ret;
  }
...
template<typename EIdType, typename ValType>
std::vector<ull> count_flops_util(const csr<EIdType,ValType>&a, const csr<EIdType,ValType>&b, int blockSizeA, bool bTranspose){
  IdxType num_rowb = bTranspose? b.nc: b.nr;
  std::vector<IdxType> nz_per_rowb(num_rowb);
  if(bTranspose){
    for(EIdType i=0;i<b.nnz;i++){
      nz_per_rowb[b.idx[i]] ++;
    }
  }else{
    for(int i=0;i<b.nr;i++){
      nz_per_rowb[i] = b.ptr[i+1]-b.ptr[i];
    }
  }
  //////////////////////////
  std::vector<ull> block_flops((a.nr+blockSizeA-1)/blockSizeA);
  for(IdxType i=0;i<a.nr;i++){
    ull cur_flops = 0;
    for(EIdType j=a.ptr[i];j<a.ptr[i+1];j++){
      cur_flops += nz_per_rowb[a.idx[j]];
    }
    block_flops[i/blockSizeA] += cur_flops;
  }
  return std::move(block_flops);
}
