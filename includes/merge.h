...
struct MergeWorker
{
  ...
  using fwriter_t = IO::FileWriter;

	ull *ptr;
	ull blocknnz_prefsum;
  ...
  MergeWorker(MergeWorker&&rhs){
    ...
    IdxType R = std::min(L+gs->blockSizeA, csra.nr), block_nr = R-L;
    ...
    for(IdxType cBid=0;cBid<gs->numBlocksB;cBid++){
      ...
      if(rd_src.getBlockStatus(cBid) & 32) return 1;
    }
    ...
    if(rd_src.getBlockStatus(0) & 128){//merge cpu results 
      ...
      profiler::Instance().cpu_data.merge_time += get_chrono_ms(t0, t1);
      profiler::Instance().cpu_data.io_time += get_chrono_ms(t1,t2);
      return 0;
    }
...
struct MergeWorker<Gs_t, true>
{
  ...
  int work(int tid, int buffer){
    ...
    IdxType L = rBid * gs->blockSizeA, R = std::min(L+gs->blockSizeA, csra.nr), block_nr = R-L;
    while(!(buffer_rd.getBlockStatus(0) & 64)) std::this_thread::yield();
    if(buffer_rd.getBlockStatus(0) & 32) return 1;
