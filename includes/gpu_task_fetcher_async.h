...
struct GPUTaskFetcherAsync
{
  ...
  uint8_t subthread_handle_task_id(int task_id){
    if(task_id < 0){
      ...
      profiler::Instance().gpu_data->h2d_time += profiler::Instance().gpu_data->timer_h2d.consume();
      if(gpu_worker->addGpuTask(m_rBid, task_id, m_buffer)){
        pipeline_fetcher._ta->notifier.notify();
      }
      return TAG_H2D;
    }
  }
  static void*compute_route(void*arg){
    ...
    try{
      while(true){
        ...
        if(gcb.free_state & tag) {
          if(tag == TAG_H2D && !all_received){ 
            ...
            prf.gpu_data->kernel_time += prf.gpu_data->timer_knl.consume();
            if(gworker.doKernel()){
              This->pipeline_fetcher._ta->notifier.notify();
            }
          }
          else if(tag == TAG_PTR){
            if(!gcb.ptr_info.pending) continue;
            ...
            prf.gpu_data->d2h_time += prf.gpu_data->timer_d2h.consume();
            if(gworker.doPtrD2H()){
              This->pipeline_fetcher._ta->notifier.notify();
            }
          }
      ...
      printf("in %s:%d, caught exception!, %s\n", __FILE__, __LINE__, e.what());
    }
    prf.gpu_data->h2d_time += prf.gpu_data->timer_h2d.consume();
    prf.gpu_data->kernel_time += prf.gpu_data->timer_knl.consume();
    prf.gpu_data->d2h_time += prf.gpu_data->timer_d2h.consume();
    auto t_end = std::chrono::steady_clock::now();
    prf.gpu_data->gpu_computer_time += get_chrono_ms(t_start, t_end);
    return nullptr;
  }
} ;
