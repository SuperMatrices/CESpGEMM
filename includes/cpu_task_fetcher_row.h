...
struct CPU_TaskFetcher_by_row
{
  ...
  static constexpr int num_workers = num_devices + 1;

  using fetch_t = TaskFetch<num_workers, int>;
  ...
  void route(){
    ErrorNotifier&notifier = pipeline_fetcher._ta->notifier;
    while(true){
      ...
      if(cBid < 0){
        auto [rowBid, buffer] = decode_row_buffer(cBid);
        change_row_and_buffer(rowBid, buffer);
      }else{
        if(mkl_computer.compute(m_rBid, m_buffer)){
          notifier.notify();
        }
        pipeline_fetcher.finish_task();
      }
    }
  }
