...
constexpr int n_omp_threads_util = 16;
...
// Constructor - create CSR matrix with specified dimensions
template<typename eidType, typename valType>
csr<eidType, valType>::csr(IdxType nr, IdxType nc, eidType nnz):
  nr(nr),nc(nc),nnz(nnz)
{
  printf("csr(%d,%d,%llu) %p\n",nr,nc,(ull)nnz, this);
  alloc(nr,nnz);
}

// Initialize CSR matrix from COO format (coordinate format)
template<typename eidType, typename valType>
void csr<eidType, valType>::init_from_coo_cpu(const coo<eidType, valType> & c, bool transposed){
  ...
  // Count non-zero elements per row using atomic operations
  ...
  // Fill idx and val arrays with column indices and values
  #pragma omp parallel for num_threads(n_omp_threads_util)
  for(eidType i=0;i<nnz;i++){
    eidType s;
    IdxType rid = tmp_row[i];
...
// Transpose matrix (multi-threaded version using OpenMP)
template<typename eidType, typename valType>
...
// Transpose matrix (single-threaded version)
template<typename eidType, typename valType>
...
// Convert from COO to vector of CSR matrices (deprecated)
template<typename EIdType, typename ValType>
...
// Extract a slice from CSC format and convert to CSR format
// Used to split matrix B into column blocks for parallel processing
