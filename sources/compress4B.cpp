...
// Priority queue for managing zero segments (sorted by length)
template<typename T>
struct Queue
{
  T* q;
  ...
  // Insert element maintaining sorted order (largest first)
  void push(T x){
    int i=len-1;
    #pragma unroll
    for(;i>0;i--){
...
enum CompCode4B Compressor4B<T,SL>::compress_with_zero_per_seg(int nElems, const T *src, int nz_threshold, int maxNsegs_perseg){
  ...
  using Pq = Queue<std::pair<int,int> >;
  // assert(maxNsegs_perseg <= 2);
  Pq q_zero_segs(maxNsegs_perseg);
  vector<Seg>segs;
  ...
  auto Consume=[this, src, &segs, &q_zero_segs, &tgt_data_bytes, &tgt_control, &tgt_anchors, &sum_values, &pref_nzeros] (u32 &total_length) -> enum CompCode4B {
    ...
    // Resize control array if needed (2 bits per element)
    ...
    for(idx=0,it=segs.begin();idx<=lastId;idx++, it++){
      ...
      for(u32 j=l;j<=r;j++){
        ...
        if(val_back>10000){
          ...
          if(nb>1) printf("elemId=%u, nb=%d, val=%u\n", elemId, nb, val_back);
        }
