...
// Zero segment: tracks length and start position
struct ZeroSeg{
  uint32_t len;
  uint32_t start;
...
// Copy vector to heap-allocated array
template<typename T>
...
// Segment: represents a contiguous range [l, r] with zero/non-zero flag
struct Seg{
  using u32=uint32_t;
  u32 l,r;
  bool is_zero;
...
template<typename T>
// Compress array with zero segments per segment
// nElems: number of elements, src: input array
// nz_threshold: minimum length to treat as zero segment
// maxNsegs_perseg: maximum zero segments per output segment
enum CompCode Compressor<T>::compress_with_zero_per_seg(int nElems, const T*src, int nz_threshold, int maxNsegs_perseg){
  ...
  // Debug helper: print queue contents
  ...
  // Lambda: Process segments to fill one output segment of length SegLen
  auto Consume = [this, src, &segments, &q_zero_segs, &tgt_data_bytes, &tgt_control, &tgt_anchors, &sum_values, &pref_nzeros, printqueue] (u32 &total_length) -> enum CompCode {
    ...
    int idx=0, lastId = 0;
    // Find where current segment ends (up to SegLen elements)
    u32 sumval_in_seg = 0;
    u32 nzeros_in_seg = 0;
    u32 rem = 0;
    ...
    // Clear queue and re-add zero segments from remaining data
    ...
    // Write zero segment headers to output
    ...
    for(idx=0, it = segments.begin(); idx<=lastId ; idx++, it++){
      ...
      for(u32 j=l;j<=r;j++){
        ...
        if(tgt_control.size() <= elemId/8){
          fprintf(stderr, "tgt_control out : eid=%d, size=%d, prefnv=%d, endnv=%d\n", elemId, (int)tgt_control.size(), previous_num_values, current_num_values);
        }
        sumval_in_seg += val;
      }
    } 
    
    // Remove processed segments, keep remainder
    if(rem){
  ...
  // Main compression loop: scan input and build segments
  u32 total_length = 0;
  u32 nnz = nElems;
  for(u32 i=0;i<nElems; ++i){
    // Non-zero sequence: create or extend segment
    if(src[i]){
      u32 start = i;
      ...
      // Zero sequence: check if long enough to be a zero segment
      else{
        if(!segments.size() || segments.back().is_zero){
  ...
  // for(int i=0;i<tgt_anchors.size()-1;++i){
  //   Anchor&anc = tgt_anchors[i];
  //   int start_idx = i*SegLen + anc.prefNZ;
  //   int data_pos = anc.srcPos;
  //   int nzhead = tgt_data_bytes[data_pos];
  //   printf("%d: start_idx = %d, nzhead=%d, data_pos=%d, data_total_len=%d\n",i,start_idx, nzhead, data_pos, tgt_data_bytes.size());
  //   for(int z=0;z<nzhead;z++){
  //     int cur_pos = data_pos+z*6;
  //     int p = tgt_data_bytes[cur_pos+1] + tgt_data_bytes[cur_pos+2]*256;
  //     int len = tgt_data_bytes[cur_pos+3]*1 + (tgt_data_bytes[cur_pos+4]<<8) + (tgt_data_bytes[cur_pos+5]<<16)+(tgt_data_bytes[cur_pos+6]<<24);
  //     printf("z %d,%d,%d\n",z,p,len);
  //   }
  // }

  // Store compressed data in member variables
